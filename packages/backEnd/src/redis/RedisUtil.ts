...
export class RedisUtil<V> {
    ...
    async set(key: string, value: V): Promise<boolean> {
        try {
            ...
            console.error(`设置 Redis Key: ${key}, Value: ${JSON.stringify(value)} 失败`, error);
            return false;
        }
    }
    ...
    async listPush(key: string, value: V, time: number): Promise<boolean> {
        try {
            ...
            console.error(`向列表推入 Redis Key: ${key}, Value: ${JSON.stringify(value)} 失败`, error);
            return false;
        }
    }
    ...
    async listPushAll(key: string, values: V[], time: number): Promise<boolean> {
        try {
            ...
            console.error(`向列表推入多个 Redis Key: ${key}, Values: ${JSON.stringify(values)} 失败`, error);
            return false;
        }
    }
    ...
    async remove(key: string, value: V): Promise<number> {
        try {
            ...
            console.error(`移除 Redis Key: ${key}, Value: ${JSON.stringify(value)} 失败`, error);
            return 0;
        }
    }
}